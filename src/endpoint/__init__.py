...
config = {
    ...
    "loggers": {
        "entrypoint": {
            "handlers": ["console"],
            "level": os.environ.get("ENTRYPOINT_LOG_LEVEL", "INFO").upper(),
            "propagate": False,
        },
        "": {
            "handlers": ["console"],
...
@click.group
@click.option("--use-config-at", "config_toml_path", default="entrypoint.config.toml")
@click.pass_context
...
def enqueue(ctx, article_url: str) -> None:
    ...
    mutation enqueue($targetProjectId: ID!, $url: String!) {
        ...
        variable_values={
            "targetProjectId": ctx.obj["url_enqueue"]["targetProjectId"],
            "url": article_url,
        },
    )
    logger.debug(result)
